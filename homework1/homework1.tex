...
Podľa \textit{Vety 4.27}~\cite{TIN}(str. č. 96) platí, že trieda deterministických bezkontextových jazykov $\mathcal{L}_2^D$ je uzavretá voči prieniku s regulárnymi jazykmi a doplnku (komplementu).
...
Pre $w \in L: w=1^p\#1^p$ pre ktoré platí podmienka $|w| \geq p$ pretože platí $2p+1 \geq p$, pričom z dôvodu podmienky $|xy| \leq p$ nastane jediný prípad a to:\\
...
\begin{enumerate}[1.)]
    ...
    \begin{minipage}{0.85\textwidth}
            $\hspace{0.6em}w \in \Sigma^*,\ \forall A,B,S \in N$ kde $S$ je počiatočný neterminál $G_P$:\\
        \begin{tabular}{r|lcl}
